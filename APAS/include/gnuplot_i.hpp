...
public:
    ...
    /// set x axis labels
    Gnuplot& set_ylabel(const std::string &label = "x", const std::string &label2 = "x2",  const std::string &scaleY2 = "2");
    /// set y axis label
    ...
    Gnuplot& plot_xy(const X& x, const Y& y, const std::string &title = "", const int aux = 0);
...
//
template<typename X, typename Y>
Gnuplot& Gnuplot::plot_xy(const X& x, const Y& y, const std::string &title,const int aux)
{
    ...
    plotfile_xy(name, 1, 2, title, aux);
...
// set the ylabel
//
Gnuplot& Gnuplot::set_ylabel(const std::string &label, const std::string &label2, const std::string &scaleY2)
{
    std::ostringstream cmdstr;

    cmd("set ytics nomirror");
    cmd("set y2tics "+ scaleY2);
    cmdstr << "set ylabel \"" << label << "\"";
    cmd(cmdstr.str());
    std::ostringstream cmdstr2;
    cmdstr2 << "set y2label \"" << label2 << "\"";
    cmd(cmdstr2.str());
                              ...
                              const int aux)
{
    ...
    if(aux == 1)
        cmdstr << "\"" << filename << "\" using " << column_x << ":" << column_y << "axis x1y1";
    if(aux == 2)
        cmdstr << "\"" << filename << "\" using " << column_x << ":" << column_y << "axis x1y2";
    //cmdstr << "set y2tics " << 0 << ", " << 400 ;
    //cmdstr << "set y2tics -100, 10";

    //cmdstr << "plot sin(1/x) axis x1y1,100*cos(x) axis x1y2";
